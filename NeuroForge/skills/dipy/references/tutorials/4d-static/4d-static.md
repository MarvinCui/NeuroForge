...
```python
kwargs = {'static_image_files': dwi_image_file, 'moving_image_files': moving_image_file, 'transform': 'trans', 'out_dir': temp_out_dir, 'out_moved': out_moved, 'out_affine': out_affine, 'save_metric': True, 'level_iters': [100, 10, 1], 'out_quality': 'trans_q.txt'}
```
...
```python
apply_trans.run(static_image_files=dwi_image_file, moving_image_files=moving_image_file, out_dir=temp_out_dir, transform_map_file=out_affine)
```
...
*Source: test_align.py:477 | Complexity: Advanced | Last updated: 2026-05-18*