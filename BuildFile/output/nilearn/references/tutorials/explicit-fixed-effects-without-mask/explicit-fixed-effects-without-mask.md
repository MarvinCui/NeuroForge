# How To: Explicit Fixed Effects Without Mask
...
Workflow: Test the fixed effects performed manually/explicitly with no mask.
...
### Step 1: 'Test the fixed effects performed manually/explicitly with no mask.'
...
```python
multi_run_model = FirstLevelModel().fit(fmri_data[0], design_matrices=design_matrices[:1])
```
...
```python
fixed_fx_contrast, fixed_fx_variance, fixed_fx_stat, _ = compute_fixed_effects(contrasts, variance)
```
...
*Source: test_first_level.py:228 | Complexity: Advanced | Last updated: 2026-05-18*