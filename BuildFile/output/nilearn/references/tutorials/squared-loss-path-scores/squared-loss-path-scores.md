# How To: Squared Loss Path Scores
...
Workflow: test squared loss path scores
...
```python
test_scores, best_w = squared_loss_path_scores(graph_net_squared_loss, X, y, mask, alphas, 0.5, np.arange(len(X)), np.arange(len(X)), {}, verbose=0)[:2]
```
...
*Source: test_space_net.py:196 | Complexity: Intermediate | Last updated: 2026-05-18*