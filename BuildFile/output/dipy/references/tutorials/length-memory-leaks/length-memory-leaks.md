...
Workflow: test length memory leaks
...
### Step 5: Assign list_refcount_after = value
...
### Step 7: Assign s_rng = np.random.default_rng(...)

```python
s_rng = np.random.default_rng(1234)
```

### Step 8: Assign NB_STREAMLINES = 10000

```python
NB_STREAMLINES = 10000
```

### Step 9: Assign streamlines = value

```python
streamlines = [s_rng.standard_normal((s_rng.integers(10, 100), 3)).astype(dtype) for _ in range(NB_STREAMLINES)]
```

### Step 10: Assign list_refcount_before = value

```python
list_refcount_before = get_type_refcount()['list']
```

### Step 11: Assign list_refcount_after = value
...
### Step 13: Assign dtype = value

```python
dtype = dtypes[i % len(dtypes)]
```

### Step 14: Call streamlines.append()
...
*Source: test_streamline.py:500 | Complexity: Advanced | Last updated: 2026-05-18*